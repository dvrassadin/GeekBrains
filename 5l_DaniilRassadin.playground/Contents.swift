import UIKit
 ...
 class TankCar: Car {
     ...
     func perform(action: Action) {
         switch action { //хотел использовать if, но он не понимает ни action == .fillTank, ни action == Action.fillTank, ни просто Action.fillTank
 ...
 var tankCar2 = TankCar(model: "Камаз", manufactureYear: 2008, tankVolume: 2500)
 tankCar1.perform(action: .fillTank(2000))
 tankCar1.switchEngine(action: .on)
 tankCar1.printDescription()
 tankCar1.switchEngine(action: .on)
 tankCar1.perform(action: .fillTank(3500))
 tankCar2.printDescription()


 var sportCar1 = SportCar(model: "Audi R8", manufactureYear: 2020, maxSpeed: 334, tires: "Michelin")
 var sportCar2 = SportCar(model: "Lotus Evora", manufactureYear: 2010, maxSpeed: 280, tires: "Bridgestone")
 sportCar1.switchWindows(action: .open)
 sportCar1.switchWindows(action: .open)
 sportCar1.perform(action: .changeTires("Goodyear"))
